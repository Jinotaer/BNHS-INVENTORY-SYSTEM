...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
  ...
  if ($stmt->execute()) {
    ...
    // Now insert into ics_items table
    $stmt = $mysqli->prepare("INSERT INTO ics_items (
    ...
    // For now, we'll need to get or create an item
    ...
    $stmt->bind_param("iiis", $ics_id, $item_id, $quantity, $inventory_item_no);
    
    if ($stmt->execute()) {
      $success = "Inventory Custodian Slip Created Successfully";
      header("refresh:1; url=ics.php");
    } else {
      $err = "Error creating item details: " . $stmt->error;
      header("refresh:1; url=ics.php");
    }
  } else {
